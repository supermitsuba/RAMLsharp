...
namespace RAMLSharp
{
    public class RAMLMapper
    {
        ...
        public RAMLMapper(IApiDescriptionGroupCollectionProvider apiDescriptionsProvider)
        {
            _apiDescriptions = apiDescriptionsProvider.ApiDescriptionGroups.Items.SelectMany(p => p.Items);
        }
        ...
        public RAMLModel WebApiToRamlModel(Uri baseUri, string title, string version, string defaultMediaTypes, string description)
        {
            ...
            foreach (var api in _apiDescriptions)
            {
                ...
                {
                    headers = api.ActionDescriptor.GetType().GetTypeInfo().GetCustomAttributes<RequestHeadersAttribute>();
                    responseBody = api.ActionDescriptor.GetType().GetTypeInfo().GetCustomAttributes<ResponseBodyAttribute>();
                }
                
                var routeModel = new RouteModel
                {
                    Verb = api.HttpMethod.ToLower(),
                    ...
                    // TODO: Description = api.Documentation
                };
